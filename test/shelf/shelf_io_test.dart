...
void main() {
  ...
  test('chunked requests are un-chunked', () async {
    await _scheduleServer(expectAsync1((request) {
      ...
      expect(request.headers, isNot(contains(HttpHeaders.transferEncodingHeader)));
      expect(
          request.read().toList(),
          completion(equals([
            [1, 2, 3, 4]
    ...
    var request = http.StreamedRequest('POST', Uri.http('localhost:$_serverPort', ''));
  ...
  test('custom response headers are received by the client', () async {
    await _scheduleServer((request) {
      return Response.ok('Hello from /', headers: {'test-header': 'test-value', 'test-list': 'a, b, c'});
    });
  ...
  test('multiple headers are received from the client', () async {
    await _scheduleServer((request) {
      return Response.ok('Hello from /', headers: {
        ...
        'requested-values-length': request.headersAll['request-values']!.length.toString(),
        'set-cookie-values': request.headersAll['set-cookie']!,
        'set-cookie-values-length': request.headersAll['set-cookie']!.length.toString(),
      });
    });
  ...
  test('custom request headers are received by the handler', () async {
    ...
    var headers = {'custom-header': 'client value', 'multi-header': 'foo,bar,baz'};
  ...
  test('supports request hijacking', () async {
    ...
    expect(response.stream.bytesToString(), completion(equals('Hello, world!')));
  });

  test('reports an error if a HijackException is thrown without hijacking', () async {
  ...
  test('a bad HTTP host request results in a 500 response', () async {
    ...
    expect(await utf8.decodeStream(socket), contains('500 Internal Server Error'));
  });
  ...
  group('date header', () {
    ...
    test('defers to header in response', () async {
      ...
      await _scheduleServer((request) {
        return Response.ok('test', headers: {HttpHeaders.dateHeader: parser.formatHttpDate(date)});
      });
  ...
  group('chunked coding', () {
    group('is added when the transfer-encoding header is', () {
      test('unset', () async {
        ...
        var response = await _get();
        expect(response.headers, containsPair(HttpHeaders.transferEncodingHeader, 'chunked'));
    ...
    test('is preserved when the transfer-encoding header is "chunked"', () async {
      await _scheduleServer((request) {
        return Response.ok(Stream.fromIterable(['2\r\nhi\r\n0\r\n\r\n'.codeUnits]),
    ...
    group('is not added when', () {
      test('content-length is set', () async {
        ...
        var response = await _get();
        expect(response.headers, isNot(contains(HttpHeaders.transferEncodingHeader)));
        expect(response.bodyBytes, equals([1, 2, 3, 4]));
      });
  ...
  test('respects the "shelf.io.buffer_output" context parameter', () async {
    ...
    await _scheduleServer((request) {
      ...
      return Response.ok(utf8.encoder.bind(controller.stream), context: {'shelf.io.buffer_output': false});
    });
  ...
  test('includes the dart:io HttpConnectionInfo in request context', () async {
    await _scheduleServer((request) {
      expect(request.context, containsPair('shelf.io.connection_info', TypeMatcher<HttpConnectionInfo>()));
  ...
  group('ssl tests', () {
    ...
    Future<HttpClientRequest> scheduleSecureGet() => sslClient.getUrl(Uri.https('localhost:${_server!.port}', ''));
    ...
    test('secure sync handler returns a value to the client', () async {
      ...
      expect(response.statusCode, HttpStatus.ok);
      expect(await response.cast<List<int>>().transform(utf8.decoder).single, 'Hello from /');
    });
...
Future<http.Response> _get({Map<String, /* String | List<String> */ Object>? headers, String path = ''}) =>
    _request((client, url) => client.getUrl(url), headers: headers, path: path);

Future<http.Response> _head({Map<String, /* String | List<String> */ Object>? headers, String path = ''}) =>
    _request((client, url) => client.headUrl(url), headers: headers, path: path);
...
Future<http.StreamedResponse> _post({Map<String, String>? headers, String? body}) {
