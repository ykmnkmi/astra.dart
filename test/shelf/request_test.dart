...
Request _request({Map<String, String>? headers, Object? body, Encoding? encoding}) {
  return Request('GET', localhostUri, headers: headers, body: body, encoding: encoding);
}
...
void main() {
  group('constructor', () {
    ...
    group('url', () {
      ...
      test('may contain slash', () {
        var request = Request('GET', Uri.parse('http://localhost/foo/bar%2f42'));
        expect(request.url, equals(Uri.parse('foo/bar%2f42')));
      });

      test('is inferred from handlerPath if possible', () {
        var request = Request('GET', Uri.parse('http://localhost/foo/bar?q=1'), handlerPath: '/foo/');
        expect(request.url, equals(Uri.parse('bar?q=1')));
      });

      test('uses the given value if passed', () {
        var request = Request('GET', Uri.parse('http://localhost/foo/bar?q=1'), url: Uri.parse('bar?q=1'));
        expect(request.url, equals(Uri.parse('bar?q=1')));
      });

      test('may be empty', () {
        var request = Request('GET', Uri.parse('http://localhost/foo/bar'), url: Uri.parse(''));
    ...
    group('handlerPath', () {
      ...
      test('may be a single slash', () {
        var request = Request('GET', Uri.parse('http://localhost/foo/bar?q=1'), handlerPath: '/');
        expect(request.handlerPath, equals('/'));
        expect(request.url, equals(Uri.parse('foo/bar?q=1')));
      });
    });
    ...
    group('errors', () {
      ...
      group('url', () {
        test('must be relative', () {
          expect(() {
            Request('GET', Uri.parse('http://localhost/test'), url: Uri.parse('http://localhost/test'));
          }, throwsArgumentError);
        });

        test('may not be root-relative', () {
          expect(() {
            Request('GET', Uri.parse('http://localhost/test'), url: Uri.parse('/test'));
          }, throwsArgumentError);
        });

        test('may not have a fragment', () {
          expect(() {
            Request('GET', Uri.parse('http://localhost/test'), url: Uri.parse('test#fragment'));
          }, throwsArgumentError);
        });

        test('must be a suffix of requestedUri', () {
          expect(() {
            Request('GET', Uri.parse('http://localhost/dir/test'), url: Uri.parse('dir'));
          }, throwsArgumentError);
        });

        test('must have the same query parameters as requestedUri', () {
          expect(() {
            Request('GET', Uri.parse('http://localhost/test?q=1&r=2'), url: Uri.parse('test?q=2&r=1'));
          }, throwsArgumentError);

          // Order matters for query parameters.
          expect(() {
            Request('GET', Uri.parse('http://localhost/test?q=1&r=2'), url: Uri.parse('test?r=2&q=1'));
          }, throwsArgumentError);
        });
      });
      ...
      group('handlerPath', () {
        test('must be a prefix of requestedUri', () {
          expect(() {
            Request('GET', Uri.parse('http://localhost/dir/test'), handlerPath: '/test');
          }, throwsArgumentError);
        });

        test('must start with "/"', () {
          expect(() {
            Request('GET', Uri.parse('http://localhost/test'), handlerPath: 'test');
          }, throwsArgumentError);
        });

        test('must be the requestedUri path if url is empty', () {
          expect(() {
            Request('GET', Uri.parse('http://localhost/test'), handlerPath: '/', url: Uri.parse(''));
          }, throwsArgumentError);
        });
      });
      ...
      group('handlerPath + url must', () {
        test('be requestedUrl path', () {
          expect(() {
            Request('GET', Uri.parse('http://localhost/foo/bar/baz'), handlerPath: '/foo/', url: Uri.parse('baz'));
          }, throwsArgumentError);
        });

        test('be on a path boundary', () {
          expect(() {
            Request('GET', Uri.parse('http://localhost/foo/bar/baz'), handlerPath: '/foo/ba', url: Uri.parse('r/baz'));
          }, throwsArgumentError);
        });
      });
    });
  ...
  group('ifModifiedSince', () {
    ...
    test('comes from the Last-Modified header', () {
      var request = _request(headers: {'if-modified-since': 'Sun, 06 Nov 1994 08:49:37 GMT'});
      expect(request.ifModifiedSince, equals(DateTime.parse('1994-11-06 08:49:37z')));
    });
  });
  ...
  group('change', () {
    ...
    group('change headers', () {
      final request = Request('GET', Uri.parse('http://localhost:8080/static/file.html'),
          protocolVersion: '2.0',
          headers: {'header1': 'header value 1'},
          url: Uri.parse('file.html'),
          handlerPath: '/static/',
      ...
      test('adding a new values', () {
        ...
        expect(r.headers, {'header1': 'header value 1', 'content-length': '0', 'a': 'A', 'b': 'B1,B2', 'c': 'C'});
        expect(r.headersAll, {
    ...
    group('with path', () {
      test('updates handlerPath and url', () {
        ...
        var request = Request('GET', uri, handlerPath: '/static/', url: Uri.parse('dir/file.html'));
      ...
      test('regression test for Issue #142', () {
        ...
        var request = Request('GET', uri, handlerPath: '/static/', url: Uri.parse('dir/'));
      ...
      test('allows changing path leading to double //', () {
        ...
        var request = Request('GET', uri, handlerPath: '', url: Uri.parse('some_base//more'));
