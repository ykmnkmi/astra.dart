...
class _TestMessage extends Message {
  _TestMessage(Map<String, /* String | List<String> */ Object>? headers, Map<String, Object>? context, Object? body,
      Encoding? encoding)
      : super(body, headers: headers, context: context, encoding: encoding);

  @override
  Message change({Map<String, String>? headers, Map<String, Object>? context, Object? body}) {
...
void main() {
  group('headers', () {
    ...
    test('null header value becomes default', () {
      ...
      expect(() => message.headersAll['h1'] = ['value1'], throwsUnsupportedError);
    });
    ...
    test('headers are immutable', () {
      ...
      expect(() => message.headersAll['h1'] = ['value1'], throwsUnsupportedError);
      expect(() => message.headersAll['h1'] = ['value2'], throwsUnsupportedError);
      expect(() => message.headersAll['h2'] = ['value2'], throwsUnsupportedError);
    });
  ...
  group('read', () {
    ...
    test('supports a Stream<List<int>> body', () {
      var controller = StreamController<Object>();
      var request = _createMessage(body: controller.stream);
      expect(request.read().toList(), completion(equals([helloBytes, worldBytes])));
  ...
  group('content-length', () {
    ...
    test('uses the content-length header for a stream body', () {
      var request = _createMessage(body: Stream<List<int>>.empty(), headers: {'content-length': '42'});
      expect(request.contentLength, 42);
    });

    test('real body length takes precedence over content-length header', () {
      var request = _createMessage(body: [1, 2, 3], headers: {'content-length': '42'});
      expect(request.contentLength, 3);
    });

    test('is null for a chunked transfer encoding', () {
      var request = _createMessage(body: '1\r\na0\r\n\r\n', headers: {'transfer-encoding': 'chunked'});
      expect(request.contentLength, isNull);
    });
  ...
  group('mimeType', () {
    ...
    test('comes from the content-type header', () {
      expect(_createMessage(headers: {'content-type': 'text/plain'}).mimeType, equals('text/plain'));
    });

    test("doesn't include parameters", () {
      expect(_createMessage(headers: {'content-type': 'text/plain; foo=bar; bar=baz'}).mimeType, equals('text/plain'));
    });
  });
  ...
  group('encoding', () {
    ...
    test('is null without a charset parameter', () {
      expect(_createMessage(headers: {'content-type': 'text/plain'}).encoding, isNull);
    });

    test('is null with an unrecognized charset parameter', () {
      expect(_createMessage(headers: {'content-type': 'text/plain; charset=fblthp'}).encoding, isNull);
    });

    test('comes from the content-type charset parameter', () {
      expect(_createMessage(headers: {'content-type': 'text/plain; charset=iso-8859-1'}).encoding, equals(latin1));
    });
    ...
    test('adds an explicit encoding to the content-type', () {
      var request = _createMessage(body: 'è', encoding: latin1, headers: {'content-type': 'text/plain'});
    ...
    test('overwrites an existing charset if given an explicit encoding', () {
      var request =
          _createMessage(body: 'è', encoding: latin1, headers: {'content-type': 'text/plain; charset=whatever'});
      expect(request.headers, containsPair('content-type', 'text/plain; charset=iso-8859-1'));
    });
  });
}
