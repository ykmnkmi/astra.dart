...
void main() {
  ...
  test('Copies a dynamic list of int elements', () async {
    ...
    expect(response.contentLength, 4);
    expect(await response.read().single, isA<List<int>>().having((values) => values, 'values', [1, 2, 3, 4]));
  });
  ...
  group('new Response.internalServerError without a body', () {
    ...
    test('preserves content-type parameters', () {
      var response =
          Response.internalServerError(headers: {'content-type': 'application/octet-stream; param=whatever'});
      expect(response.headers, containsPair('content-type', 'text/plain; param=whatever'));
    });
  });
  ...
  group('expires', () {
    ...
    test('comes from the Expires header', () {
      expect(Response.ok('okay!', headers: {'expires': 'Sun, 06 Nov 1994 08:49:37 GMT'}).expires,
  ...
  group('lastModified', () {
    ...
    test('comes from the Last-Modified header', () {
      expect(Response.ok('okay!', headers: {'last-modified': 'Sun, 06 Nov 1994 08:49:37 GMT'}).lastModified,
          equals(DateTime.parse('1994-11-06 08:49:37z')));
    });
  });
